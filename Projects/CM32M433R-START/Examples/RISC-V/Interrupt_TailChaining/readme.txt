1、功能说明
	...
	其中TIM2超时中断为非向量中断，优先级为1，并在超时后将翻转LED D6，开启TIM3定时器，打印中断计数器
	其中TIM3超时中断为非向量中断，优先级为1，并在超时后将翻转LED D7; 关闭TIM3定时器，开启TIM4定时器，打印中断计数器
	其中TIM4超时中断为非向量中断，优先级为1，并在超时后将翻转LED D8; 关闭TIM4定时器，打印中断计数器
...
3、使用说明
	...
	7、USART配置如下：
		...
		- UART4_Tx.PD0
                - UART4_Rx.PD1
        8、通过Micro USB连接开发板，确保J6、J7跳帽短接；或使用外部USB Serial工具连接TX4,RX4管脚；

	使用方法：
		1.编译后下载程序复位运行；
	    	2.程序运行后，LED D6/D7/D8将以0.5秒间隔进行闪烁，同时log串口将输出中断计数器，由于中断咬尾特性，
