1、功能说明
	1、TIM1 CH4触发ADC1  ADC2同步注入采样OPA1 OPA2的电压数据，TIM8 CH4触发ADC3  ADC4同步注入采样OPA3 OPA4的电压数据，并且TIM1 TIM8输出受COMP1和COMP7刹车控制，刹车发生后TIM1 TIM8停止pwm输出，TIMx CH4触发ADC的注入采样停止。
2、使用环境
...
4、注意事项
	此用例会关闭jtag功能将部分io用于tim，如要进行调试注释GPIO_ConfigPinRemap(GPIO_RMP_SW_JTAG_SW_ENABLE, ENABLE);即可