...
unsigned char qmi8658_write_reg(unsigned char reg, unsigned char value)
{
	...
	while(ret && (retry++ < 5))
	{
		ret =sensor_interface_iic_write_cmd(g_imu.slave<<1, pdata, 2);
	}
	return ret;
}
...
unsigned char qmi8658_write_regs(unsigned char reg, unsigned char *value, unsigned char len)
{
	...
	*pdata = reg;
	...
	while(ret && (retry++ < 5))
	{
		ret = sensor_interface_iic_write_cmd(g_imu.slave<<1, pdata, len + 1);
	}
...
unsigned char qmi8658_read_reg(unsigned char reg, unsigned char* buf, unsigned short len)
{
	...
	while(ret && (retry++ < 5))
	{
		if(sensor_interface_iic_write_cmd(g_imu.slave<<1, &reg, 1) == 0)
		{
			ret = sensor_interface_iic_read_cmd(g_imu.slave<<1, buf, len);
		}
	}
	return ret;
}
...
unsigned char qmi8658_get_id(void)
{
	...
	//while(iCount<2)
	//{
		g_imu.slave = QMI8658_SLAVE_ADDR_H;//qmi8658_slave[iCount];
		retry = 0;
