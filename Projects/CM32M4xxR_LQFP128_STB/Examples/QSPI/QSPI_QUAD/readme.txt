1、功能说明
    通过QSPI接口操作板上的串行Flash（GD25Q40C）芯片，分别使用双线和四线模式对Flash进行读写，并打印读取的数据。
...
3、使用说明
    ...
    使用方法：
		...
		2、紧接着输出两包数据，数据的内容是十六进制格式显示的从Flash中读取的同一页（256 Bytes）的数据
		3、输出每包数据前会输出读取本包数据的模式：两线（2-line）或四线（4-line）
		4、最后输出读写数据的比对结果，成功输出“Pass!”，失败输出“Fail!”
