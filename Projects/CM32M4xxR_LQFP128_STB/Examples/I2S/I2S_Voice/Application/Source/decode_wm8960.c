...
#define WM8960_I2C_ADDRESS          (0x34u)//(0x1Au)
...
static I2C_Module* i2c_ptr = NULL;
                                       ...
                                       mtb_wm8960_adc_dac_sample_rate_t sample_rate)
{
    ...
    if ((R <= 5) || (R >= 13))
    {
        return CY_RSLT_TYPE_BAD_ARG;//MTB_RSLT_WM8960_BAD_ARG;
    }
...
cy_rslt_t _i2c_master_send(I2C_Module* i2c_inst, uint8_t addr, uint8_t* data, int len)
{
    ...
    {
        if ((I2CTimeout--) == 0)
            return CY_RSLT_TYPE_EV5;
    };

    I2C_SendAddr7bit(i2c_inst, addr, I2C_DIRECTION_SEND);

    I2CTimeout = I2C_FLAG_TIMOUT;
    while (!I2C_CheckEvent(i2c_inst, I2C_EVT_MASTER_TXMODE_FLAG)) /* EV6 */
    {
        if ((I2CTimeout--) == 0)
            return CY_RSLT_TYPE_EV6;
    };

    /* Send data */
    while (len-- > 0)
    {
        I2C_SendData(i2c_inst, *sendBufferPtr++);

        I2CTimeout = I2C_FLAG_TIMOUT;
        while (!I2C_CheckEvent(i2c_inst, I2C_EVT_MASTER_DATA_SENDING)) /* EV8 */
        {
            if ((I2CTimeout--) == 0)
                return CY_RSLT_TYPE_EV8;
        };
    };
...
// mtb_wm8960_init
//--------------------------------------------------------------------------------------------------
cy_rslt_t mtb_wm8960_init(I2C_Module* i2c_inst, mtb_wm8960_features_t features)
{
...
cy_rslt_t mtb_wm8960_write(mtb_wm8960_reg_t reg, uint16_t value)
{
    ...
    result = _i2c_master_send(i2c_ptr, WM8960_I2C_ADDRESS, buf, sizeof(buf));
    if (result == CY_RSLT_SUCCESS)
    {
